...
- [Mobile app data](#mobile-app-data)
- [Inspecting HTTPS Traffic](#inspecting-https-traffic)
...
Extract domain names via certificate Server Name Indication (SNI):
```
...
Sessions using weak encryption protocols:
```
...
Decrypting TLS traffic (SSL/TLS 1.2 and prior without PFS) using a known private key:
```
tshark -r SSL-decryption.pcap -q -o "ssl.keys_list:192.168.56.101,443,http,server.pem" -z "follow,ssl,ascii,2"
```

Decrypting TLS 1.3 with PFS (or TLS 1.2 and prior with PFS) via [SSLKEYLOGFILE environment variable](https://wiki.wireshark.org/tls):
```
# On Windows:
set SSLKEYLOGFILE=%USERPROFILE%\Desktop\keylogfile.txt   ### or use setx to set the variable persistently & globally!
start firefox  ### or chrome/brave/edge

# On macOS:
export SSLKEYLOGFILE=$HOME/keylogfile.txt
open -a /Applications/Google\ Chrome.app/  ### or firefox/brave/edge

# On Linux:
export SSLKEYLOGFILE=$HOME/keylogfile.txt
firefox  ### or chrome/brave/edge
```

With the environment variable set, you can now decrypt TLS 1.3/PFS traffic generated by firefox/chrome/brave/edge, using the queries below.
<br>
<br>
**Note 1:** Some websites (such as Google, Facebook, Instagram) use the UDP-based HTTP2/3 QUIC protocol instead of TCP/HTTPS, which is not as consistently decryptable due to the experimental status of the Google/IETF QUIC protocol spec. As a workaround (until the IETF QUIC standard is more widely adopted and support by Wireshark/tshark), go to `chrome://flags` (or `brave://flags` for Brave, or `about:config` for Firefox), search for "QUIC", and Disable support for the experimental QUIC protocol. Alternately, you can also block UDP ports 80/443 outbound at the perimeter, or via `iptables` if performing a MITM attack.
<br>
<br>
**Note 2:** For consistent reproducability, start packet captures _before_ browsing to any target sites. If needed, clear session keyfile and start over, and try starting the capture before opening browser (browser may cache sessions for some sites, even after tab is closed).
<br>
<br>

Decrypting GET/POST requests:
```
tshark -r capture.cap -o tls.keylog_file:~/keylogfile.txt -o tls.desegment_ssl_records:TRUE -o tls.desegment_ssl_application_data:TRUE -V | egrep "GET /|POST /"
```

Decrypting HTTP header data:
```
tshark -r capture.cap -o tls.keylog_file:~/keylogfile.txt -o tls.desegment_ssl_records:TRUE -o tls.desegment_ssl_application_data:TRUE -V -Y "http || http2 || http3" | egrep "Header: "
```

Decrypting authorization headers (possibly containing creds/hashes):
```
tshark -r capture.cap -o tls.keylog_file:~/keylogfile.txt -o tls.desegment_ssl_records:TRUE -o tls.desegment_ssl_application_data:TRUE -V -Y "http || http2 || http3" | egrep "Header: authorization:"
```

Decrypting interesting headers (including auth/keys/cookies):
```
tshark -r capture.cap -o tls.keylog_file:~/keylogfile.txt -o tls.desegment_ssl_records:TRUE -o tls.desegment_ssl_application_data:TRUE -V -Y "http || http2 || http3" | egrep "Header: (:method:|:authority:|:path:|authorization: |cookie: )"
```

Decrypting HTTP POST Form Data (including credentials) (**Note:** rarely (i.e. for Apple ID logins), form data may show up under `String value:` instead of `Form item:`):
```
tshark -r capture.cap -o tls.keylog_file:~/keylogfile.txt -o tls.desegment_ssl_records:TRUE -o tls.desegment_ssl_application_data:TRUE -V | egrep "Form item: "
```

Decrypting all HTTP POST parameter data:
```
tshark -r capture.cap -o tls.keylog_file:~/keylogfile.txt -o tls.desegment_ssl_records:TRUE -o tls.desegment_ssl_application_data:TRUE -V -Y http2.headers.method==POST -Tfields -e http2.data.data
```

Determining GQUIC version(s) in use:
```
tshark -r capture.cap -o tls.keylog_file:~/keylogfile.txt -o tls.desegment_ssl_records:TRUE -o tls.desegment_ssl_application_data:TRUE -V | grep -i google_quic_version
```

Redirecting traffic through an attacker-controlled proxy server (to capture encrypted traffic remotely):
```
google-chrome --proxy-server="socks5://YOUR_PROXY:8080"
```
<br>
<br>
<br/>
